...
class SessionService:
    ...
    def create_session(self, data: CreateSessionRequest) -> ResearchSession:
        """Create and store a new research session."""
    ...
    def get_session(self, session_id: str) -> ResearchSession:
        """Retrieve a session by ID or raise ResourceNotFoundException."""
    ...
    def list_sessions(self) -> tuple[list[ResearchSession], int]:
        """Retrieve all sessions with total count."""
        sessions = self.repository.list_all()
        return sessions, len(sessions)

    def update_session(
        self, session_id: str, data: UpdateSessionRequest
    ) -> ResearchSession:
        """Update session fields and validate state machine transitions."""
